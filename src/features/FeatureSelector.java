 ...
 * @author magdy
 */
public class FeatureSelector extends ThreadObserver {
 ...
 //   private static String data_set_file_path = null;
    private static File output_file = new File("outFFF");
    //pmi or chi2
    private static FeatureSelectionMetricEnum metric =  metric = FeatureSelectionMetricEnum.getMetricByKey("chi2");
    private static int selected_features_count = 100;
...
*/
    ...
    public void printOutput(final TMap<String, Double> all_features_scores) {
        PrintStream os = null;
        if (output_file != null) {
            try {
    ...
    public  TMap<String, Double> all_features_scores = null;

    @Override
    public void allThreadsFinished(List<DataSetFileEntry> _data) {
        readDataSetFile(_data);
        // execute feature selection
         all_features_scores = FeatureSelectionMetric.getInstance(
                metric,
